...
class ControllerCatalogInformation extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (isset($this->request->post['bottom'])) {
			...
			$data['blog'] = $information_info['blog'];
		} else {
			$data['blog'] = 0;
		}
		...
		$data['rubriki'] = $this->model_catalog_rubriki->getCategories($filter_data1);
                
                
                if (isset($this->request->post['rubriki_id'])) {
			$data['rubriki_id'] = $this->request->post['rubriki_id'];
		} elseif (!empty($information_info)) {
			$data['rubriki_id'] = $this->model_catalog_information->getInformarionMainRubrikiId($this->request->get['information_id']);
		} else {
			$data['rubriki_id'] = 0;
		}
