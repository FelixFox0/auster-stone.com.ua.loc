...
class ModelCatalogReview extends Model {
	public function addReview($product_id, $data) {
		$this->event->trigger('pre.review.add', $data);
                if(isset($data['information'])){
                    $this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '0', information_id = '" . (int)$product_id . "', text = '" . $this->db->escape($data['text']) . "', rating = '5', date_added = NOW()");
                }else{
                    $this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int)$this->customer->getId() . "',information_id = '0', product_id = '" . (int)$product_id . "', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', date_added = NOW()");
                }
		...
		if ($this->config->get('config_review_mail')) {
			...
			$this->load->model('catalog/information');
                        if(isset($data['information'])){
                            $product_info = $this->model_information_information->getProduct($product_id);
	...
	public function getTotalInformationByProductId($information_id) {
