...
class ModelCustomerCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET address_id = '" . (int)$address['address_id'] . "', customer_id = '" . (int)$customer_id . "', fio = '" . $this->db->escape($address['fio']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? json_encode($address['custom_field']) : '') . "'");
	...
	public function getTotalCustomers($data = array()) {
		...
		if (!empty($data['filter_name'])) {
			$implode[] = "fio LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
