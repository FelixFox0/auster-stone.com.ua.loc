...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET blog = '". (isset($data['blog']) ? (int)$data['blog'] : 0) ."' ,sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "'");
		...
		if (isset($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'information_id=" . (int)$information_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
                ...
                if(isset($data['rubriki_id']) && $data['rubriki_id'] > 0) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "information_to_rubriki WHERE information_id = '" . (int)$information_id . "'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "information_to_rubriki SET information_id = '" . (int)$information_id . "', rubriki_id = '" . (int)$data['rubriki_id'] . "', main_rubriki = 1");
		} 
	...
	public function editInformation($information_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "information SET blog = '". (isset($data['blog']) ? (int)$data['blog'] : 0) ."' , sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "' WHERE information_id = '" . (int)$information_id . "'");
                if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "information SET image = '" . $this->db->escape($data['image']) . "' WHERE information_id = '" . (int)$information_id . "'");
		}
	...
	public function getTotalInformationsByLayoutId($layout_id) {
        ...
        public function getInformarionMainRubrikiId($information_id){
            $query = $this->db->query("SELECT rubriki_id FROM " . DB_PREFIX . "information_to_rubriki WHERE information_id = '" . (int)$information_id . "' AND main_rubriki = '1' LIMIT 1");

		return ($query->num_rows ? (int)$query->row['rubriki_id'] : 0);
        }
}